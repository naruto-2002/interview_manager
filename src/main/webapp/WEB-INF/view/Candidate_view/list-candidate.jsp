...
<div class="container">
    ...
    <div class="text-right">
        <a  type="submit" href="/candidate/addCandidateForward" class="btn btn-primary action-button btn-sm"  id="addButton">Add Candidate</a>
    </div>
        ...
        <c:forEach items="${candidates}" var="candidate" >
            <tr>
                ...
                <td class="action-buttons">
                    <a type="submit" href="/candidate/candidateDetail?id=${candidate.id}" class="btn btn-sm btn-info action-button">View</a>
                    <a type="submit" href="/candidate/updateForward?id=${candidate.id}" class="btn btn-sm btn-warning action-button">Edit</a>
                    <button class="btn btn-sm btn-danger action-button" onclick="remove(${candidate.id})">Delete</button>
                </td>
            </tr>
        </c:forEach>
        </tbody>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            <c:if test="${p.number>0}">
                <li class="page-item">
                    <a class="page-link" href="/candidate/searchCandidate?page=${p.number-1}&${requestParams}" aria-label="Previous">
            ...
            <c:if test="${p.totalPages!=0}">
                <c:forEach begin="0" end="${p.totalPages - 1}" var="i">
                    <li class="page-item ${i == p.number ? 'active' : ''}">
                        <a class="page-link" href="/candidate/searchCandidate?page=${i}&${requestParams}">${i + 1}</a>
                    </li>
                </c:forEach>
            </c:if>
            ...
            <c:if test="${p.number<p.totalPages-1}">
                <li class="page-item">
                    <a class="page-link" href="/candidate/searchCandidate?page=${p.number+1}&${requestParams}" aria-label="Next">
